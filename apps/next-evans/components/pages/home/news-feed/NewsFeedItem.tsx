...
const NewsFeedItem: React.FC<Props> = ({
  ...
  return (
    <FlexLink
      ...
      type={type}
      className={cx(
        `flex hover:bg-gray-200 transition duration-100`,
        alt ? `bg-white` : `bg-flgray-100`,
      )}
    >
      <div
      ...
      >
        ...
        <p className="text-center text-xs sm:text-sm text-flgray-600 antialiased sans-wider capitalize">
          {month} {day}
        </p>
      </div>
      <div className="flex-grow border-l border-gray-400 py-3 pl-4 sm:pl-6 pr-6 sm:pr-8 sm:flex items-center">
        <NewsFeedIcon className="hidden sm:flex mr-6" type={type} />
        <div>
          <h3
