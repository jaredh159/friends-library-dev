...
describe(`modernizeWords()`, () => {
  ...
  it(`creates a lint to replace "intercourse"`, () => {
    ...
    expect(results[0]).toEqual({
      ...
      type: `error`,
      rule: `modernize-words`,
      message: `"intercourse" should be replaced in modernized editions (communication, interaction, conversation, commerce, dealings, exchange, fellowship, communion, contact, correspondence, etc.)`,
      fixable: false,
    });
  });
